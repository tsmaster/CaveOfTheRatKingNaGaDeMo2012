...
public class GameMap {
	...
	private void createObject(Json.Object obj, int x, int y) {
		...
		if (objectType.equalsIgnoreCase("portal")) {
			...
			String finalCombat = obj.getString("final_combat", "false");
			CombatMapMarker ratKing = new CombatMapMarker(enemyName, combatMap, this, finalCombat.equalsIgnoreCase("true"));
	...
	public Encounter getRandomEncounter() {
		...
		for (EncounterDescription ed : randomEncounterDescriptions) {
			...
			if (r < 0) {
				...
				int numRats = 2;
				int numCaptains = 0;
				if (ed.getName().equalsIgnoreCase("rats_1")) {
					numRats = 1;
				} else if (ed.getName().equalsIgnoreCase("rats_2")) {
					numRats = 2;
				} else if (ed.getName().equalsIgnoreCase("rats_3")) {
					numRats = 3;
					numCaptains = 1;
				}
				e.setGold(numRats * 30).setExperience(numRats*200);
